...
% PRIORITY: [CRITICAL]
% ============================================
\section{Numerical Mathematics / 数值计算 (DEN) [SURVIVAL]}

\begin{warnbox}[\textbf{[WARN]} PRIORITY: SURVIVAL MODE / 优先级：求生模式]
\textbf{[CN]} 数值计算优先级为 \textbf{CRITICAL}。 \textbf{只背}: 牛顿法 + 收敛条件！
\textbf{[EN]} Numerical priority is \textbf{CRITICAL}. \textbf{ONLY memorize}: Newton-Raphson + Convergence condition!\\
\textbf{Goal}: Write the formula correctly and shut up. 目标：写对公式然后闭嘴。
