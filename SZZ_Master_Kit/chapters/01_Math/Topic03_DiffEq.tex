...
\section{Differential Equations / 微分方程 (DEN) [SURVIVAL]}
\label{sec:differential-equations}

\begin{warnbox}[\textbf{[WARN]} PRIORITY: SURVIVAL MODE / 优先级：求生模式]
\textbf{[CN]} DEN 优先级为 \textbf{CRITICAL}。 \textbf{只背}: 分离变量法 + 一阶线性解！
\textbf{[EN]} DEN priority is \textbf{CRITICAL}. \textbf{ONLY memorize}: Separable ODE + First-order linear solution!\\
\textbf{Goal}: Don't solve complex integrals. 目标：别去解复杂的积分。
\end{warnbox}
