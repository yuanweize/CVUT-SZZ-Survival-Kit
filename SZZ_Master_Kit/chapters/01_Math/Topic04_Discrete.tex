...
% PRIORITY: [CRITICAL]
% ============================================
\section{Discrete Math \& Graphs / 离散数学与图论 (DMG) [SURVIVAL]}

\begin{warnbox}[\textbf{[WARN]} PRIORITY: SURVIVAL MODE / 优先级：求生模式]
\textbf{[CN]} DMG 优先级为 \textbf{CRITICAL}。 \textbf{只背}: 集合运算 + 基数 + 图连通性！\\
\textbf{[EN]} DMG priority is \textbf{CRITICAL}. \textbf{ONLY memorize}: Set ops + Cardinality + Graph connectivity!\\
\textbf{Goal}: Definitions only. 目标：只背定义。
