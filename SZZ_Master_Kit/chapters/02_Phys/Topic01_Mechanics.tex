...
\section{Mechanics / 力学 (PH1)}
\label{sec:mechanics}

\begin{warnbox}[\textcolor{orange}{\textbf{[CAUTION]}} PRIORITY: FOCUS MODE / 优先级：专注模式]
\textbf{[CN]} 力学优先级为 \textbf{CAUTION}。 \textbf{重点}: F=ma (牛顿第二定律) + 谐振动！\\
\textbf{[EN]} Mechanics priority is \textbf{CAUTION}. \textbf{Focus}: F=ma + Harmonic Motion!\\
\textbf{Thesis Link}: Accelerometer measures $a \to F=ma$. 加速度计测量的是力.
\end{warnbox}
